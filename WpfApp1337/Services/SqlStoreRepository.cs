...
namespace ApplianceStoreIS.Services
{
    public class SqlStoreRepository : IStoreRepository
    {
        ...
        {
            appConnection = GetConnectionString();
            masterConnection = GetMasterConnectionString(appConnection);
        ...
        {
            using (var conn = new SqlConnection(appConnection))
            using (var cmd = new SqlCommand("INSERT INTO Orders (ProductName, Quantity, OrderDate, Status) VALUES (@ProductName,@Quantity,@OrderDate,@Status); SELECT CAST(SCOPE_IDENTITY() as int);", conn))
            {
        ...
        {
            var connectionString = ConfigurationManager.ConnectionStrings["ApplianceStoreISDbEntities1"]?.ConnectionString;
            if (string.IsNullOrWhiteSpace(connectionString))
            {
                throw new InvalidOperationException("В App.config не найдена строка подключения 'ApplianceStoreISDbEntities1'.");
            }

            return connectionString;
        }

        private static string GetMasterConnectionString(string sourceConnection)
        {
...
END
IF COL_LENGTH('Orders', 'UserLogin') IS NOT NULL
BEGIN
    ALTER TABLE Orders DROP COLUMN UserLogin;
END";
